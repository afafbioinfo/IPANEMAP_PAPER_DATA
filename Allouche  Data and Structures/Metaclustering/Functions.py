...
mpl.style.use('seaborn')# for grid + background
#*************************!!!!!!!! *************************!!!!!!!!!!!!!!!!!!!
...
def plotClusteringDistribution(lenconstraint,Folder_name,Lenrna):
	...
	print "ppp",clusters
        ...
        #orig=ax.matshow(fit_data, cmap=plt.cm.Blues)
        #fig.colorbar(orig)
        #plt.title("Base pairs eucledian distance between probing conditions") 
        #ax.set_xticklabels(model.row_labels_)
        #ax.set_xticks(np.arange(len(model.row_labels_)))# to show all labels
	#ax.set_yticklabels(model.column_labels_)
        #ax.set_yticks(np.arange(len(model.column_labels_)))
        #plt.show()
        ...
        flatui=["#66C2A5","#457d6b","#365d50","#263e36"]#,"#17221e"]#,"#FC8D62"]#,"#66C2A5", "#95a5a6","#E78AC3", "#A6D854", "#FC8D62"]
        mycmap = ListedColormap(sns.color_palette(flatui).as_hex())
        
        b2=ax.matshow(np.outer(np.sort(model.row_labels_) + 1,np.sort(model.column_labels_) + 1), cmap=mycmap)
        b2=ax.matshow(fit_data, cmap=mycmap)
        #b2.ax.tick_params(labelsize=18) 
        cbar=fig.colorbar(b2,ticks=[0,40])
        cbar.set_label(label='Euclidean distance',size=14)
	...
	rows=[Dic[label][4:] for label in np.argsort(model.row_labels_)]
        columns=[Dic[label][4:] for label in np.argsort(model.column_labels_)]
        #rows=["1M7MgCE","NMIAMgCE","BzCNMg", "DMSMg","Thermo","NaiMg","CMCTMg","NMIA","NMIAMg","1M7ILU","1M7ILUMg","1M7ILU3Mg", "1M7ILU3", "NMIACE","BzCN","Nai","1M7CE"]
	#colums=["1M7MgCE","NMIAMgCE","BzCNMg", "DMSMg","Thermo","NaiMg","CMCTMg","NMIA","NMIAMg","1M7ILU","1M7ILUMg","1M7ILU3Mg", "1M7ILU3", "NMIACE","BzCN","Nai","1M7CE"]
        #print rows
	...
	amax = np.amax(adist)
	adist /= amax
	mds = manifold.MDS(n_components=2, dissimilarity="precomputed", n_init=4,random_state=None)
	...
	k=0
        ["didyThermo","didy1M7Mg", "didyNMIA", "didyNMIAMg", "didyNMIAMgCE", "didyBzCNMg","didyCMCTMg",  "didyNaiMg","didyDMSMg","didy1M7ILU3","didy1M7ILU","didy1M7ILU3Mg","didy1M7ILUMg"]
 
	print "lol", Dic.values()
        listoriented=['NaiMg', 'NMIAMgCE','BzCN', 'BzCNMg', 'Nai', '1M7ILUMg', '1M7ILU3Mg', '1M7ILU3', '1M7ILU', '1M7Mg', '1M7', 'CMCTMg', 'NMIAMg', 'DMSMg', 'NMIA','Thermo','NMIACE']
	listoriented=["Thermo","1M7Mg", "NMIA", "NMIAMg", "NMIAMgCE", "BzCNMg","CMCTMg",  "NaiMg","DMSMg","1M7ILU3","1M7ILU","1M7ILU3Mg","1M7ILUMg"]
	for label, x, y in zip(Dic.values(), coords[:, 0], coords[:, 1]):
		...
		#	state="left"
			...
			label[4:],
			xy = (x, y), xytext = Pos,
			textcoords = 'offset points', ha = state, va = topbottom, fontsize=12,
			bbox = dict(boxstyle = 'round,pad=0.05', fc = "#66C2A5", alpha = 0.3))
